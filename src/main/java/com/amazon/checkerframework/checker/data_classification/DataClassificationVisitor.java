...
/**
 * A visitor class is responsible for issuing errors. DCC extends the default
 * visitor to change the
...
public class DataClassificationVisitor
     ...
     *
     * @param checker the checker implementation itself. Contains hooks into the
     *                ATF, etc.
     */
    ...
    /**
     * Overrides the default lower bound for exception parameters. By default, this
     * is top
     * (@Critical). This prevents DCC from defaulting exception parameters
     * to @Public. Overriding
     * this method changes the expected lower bound to be @Public, permitting that
     * default.
     *
    ...
    // return Collections.singleton(AnnotationBuilder.fromClass(elements,
    // Public.class));
    // }
    @Override
    protected AnnotationMirrorSet getExceptionParameterLowerBoundAnnotations() {
        AnnotationMirrorSet annotationSet = new AnnotationMirrorSet();
        annotationSet.add(AnnotationBuilder.fromClass(elements, Public.class)); // Add your desired annotation here
        return annotationSet;
    }

    /**
     * Overrides the default upper bound for thrown exceptions. By default, this is
     * the same as the
     * result of getExceptionParameterLowerBoundAnnotations, which defaults to top.
     * This is
     * overridden to keep the default top, since that other method is also
     * overridden.
     *
    ...
    //     AnnotationMirrorSet annotationSet = new AnnotationMirrorSet();
    //     annotationSet.add(AnnotationBuilder.fromClass(elements, Critical.class)); 
    //     return annotationSet;
    // }
    /**
     * This is a bit of a hack. This method controls whether the {@link
     * org.checkerframework.common.basetype.BaseTypeValidator} actually issues
     * type.invalid errors,
     * and is typically guarded by a subtyping check. I disabled it here because the
     * framework
     * doesn't provide a way to specify defaults on type declarations.
     *
     * @see <a
     *      href=
     *      "https://groups.google.com/forum/#!msg/checker-framework-dev/vk2V6ZFKPLk/v3hENw-e7gsJ">
     *      this discussion</a>
     ...
     *
     * <ol>
     * <li>If the varType is @Poly and the valueType is some non-poly type, replace
     * the varType
     * with the class bound from the underlying type.
     * <li>Similarly, if the valueType is @Poly and the varType is some non-poly
     * type, replace the
    ...
    /**
     * Replaces the polymorphic annotation in atm with the least upper bound of the
     * annotations
     * associated with the underlying classes, and any other resolvable polymorphic
     * annotations in
     * scope.
     *
     * @param atm       an annotated type mirror with a polymorphic annotation in
     *                  DCC's type system which
     *                  is a candidate for replacement
     * @param localTree a tree in the same scope as atm; used to determine the
     *                  enclosing method
     */
    private void replacePoly(final AnnotatedTypeMirror atm, final Tree localTree) {
        List<TypeMirror> underlyingTypes = new ArrayList<>();

        // Collect the types associated with the parameters and receiver of the
        // enclosing method;
        // mirrors polymorphic resolution. Ignore @poly("use") by using
        // isPolyWithNoArgs(), which
        // only looks for @PolyClassification with no arguments.
        MethodTree enclosingMethod = TreePathUtil.enclosingMethod(atypeFactory.getPath(localTree));
        if (enclosingMethod != null) {
            ExecutableElement execElem = TreeUtils.elementFromDeclaration(enclosingMethod);
            AnnotatedTypeMirror.AnnotatedExecutableType methodSignature = atypeFactory.fromElement(execElem);
        ...
        // Always include the underlying type of the input ATM. For fields and other
        // non-method code
        // (or
        // code in methods with no polymorphic arguments), this will be the only type
        // used for
        // resolution.
        underlyingTypes.add(atm.getUnderlyingType());

        // The annotation that will replace the polymorphic annotation in the
        // computation.
        ...
        for (TypeMirror underlyingType : underlyingTypes) {
            ...
            if (underlyingClassElem != null) {
                // And then use the ATF to produce the type the user wrote on the class - and
                // those
                // inferred by DataClassificationTypeFactory#fromElement.
                AnnotatedTypeMirror classDefaultType = atypeFactory.fromElement(underlyingClassElem);
                if (classDefaultType != null) {
                    // If the class does have a default type (all should...), try to read an DCC
                    // annotation from it.
                    AnnotationMirror classDefaultAnnotation = classDefaultType.getAnnotationInHierarchy(
                    ...
                    if (classDefaultAnnotation != null) {
                        ...
                        // so far.
                        finalClassAnnotation = atypeFactory
        ...
        // Take the final LUB and then replace the original polymorphic annotation with
        // it.
        atm.replaceAnnotation(finalClassAnnotation);
    }

    /**
     * Checks whether the given annotation contains an DCC @PolyClassification
     * annotation. Note that
     * this only returns true if the annotation is @PolyClassification with no
     * arguments.
     *
    ...
    /**
     * Checks whether the given annotation is NOT @PolyClassification. This is NOT
     * the inverse of
     * isPolyWithNoArgs, because they both return false on an @PolyClassification
     * annotation with an
    ...
    /**
     * Searches through a type for user-written (i.e. non-Public) annotations.
     * Returns true if any
     * are found.
     *
    ...
    private boolean hasNonPublic(final AnnotatedTypeMirror atm) {
        ...
        switch (atm.getKind()) {
            case DECLARED:
                AnnotatedTypeMirror.AnnotatedDeclaredType atmD = (AnnotatedTypeMirror.AnnotatedDeclaredType) atm;
    ...
    /**
     * DCC defaults implicit upper bounds of type variables to @Public. This is the
     * correct thing to
     * do in unannotated code, but causes false positive errors whenever a user
     * writes a type
     * annotation on the upper bound of a type variable (for instance, by declaring
     * a list of highly
     * confidential strings). Since this is common, this code disables that check
     * for user-written
     * annotations.
     */
    ...
    protected void checkTypeArguments(
        ...
        if (!newTypeArgs.isEmpty()) {
            super.checkTypeArguments(toptree, newParamBounds, newTypeArgs, typeargTrees,typeOrMethodName,paramNames);
        }
    }

    /**
     * Skip the standard subtyping check on method calls iff the enclosing class of
     * the
     * declaration's type is a supertype of the methodCallReceiver.
     *
     * <p>
     * In other words, permit the enclosing class' annotation to substitute for the
     * definition.
     *
     * <p>
     * This avoid needing to write corresponding receiver annotations on every
     * method in a class
     * with an DCC annotation, which is super annoying for users.
     */
    @Override
    protected boolean skipReceiverSubtypeCheck(final MethodInvocationTree node,
        ...
        Element enclosingClass = ElementUtils.enclosingTypeElement(definition);
        AnnotatedTypeMirror classType = atypeFactory.getAnnotatedType(enclosingClass);

        AnnotationMirror classAnno = classType.getAnnotationInHierarchy(atypeFactory.getCanonicalPublicAnnotation());
        AnnotationMirror methodCallReceiverAnno = methodCallReceiver.getAnnotationInHierarchy(
                atypeFactory.getCanonicalPublicAnnotation());
        ...
        return atypeFactory.getQualifierHierarchy().isSubtypeQualifiersOnly(methodCallReceiverAnno, classAnno);
    }
}
